...
兩階段流程：
- POST /api/misc-receipts               建立 status='pending'
- POST /api/misc-receipts/{id}/sign     上傳簽名/照片 → status='signed'
"""
...
from schemas.misc_receipts import (
...
# ─── Pydantic schemas ────────────────────────────────────────────────────
class MiscReceiptBase(BaseModel):
    receipt_date: date
    ...
    receipt_number: Optional[str] = Field(None, max_length=60)
    notes: Optional[str] = None

    @field_validator("receipt_date")
    @classmethod
    def guard_receipt_date(cls, v: date) -> date:
        # 共用金流日期守衛：禁未來日、限制回補 90 天，避免把收入搬到錯誤月份。
        return validate_payment_date(v, back_limit_days=RECEIPT_DATE_BACK_LIMIT_DAYS)

    @field_validator("payer_name", mode="before")
    @classmethod
    def strip_payer(cls, v):
        return v.strip() if isinstance(v, str) else v

    @field_validator("description", "receipt_number", "notes", mode="before")
    @classmethod
...
class MiscReceiptCreate(MiscReceiptBase):
...
class MiscReceiptUpdate(BaseModel):
    receipt_date: Optional[date] = None
    payer_name: Optional[str] = Field(None, min_length=1, max_length=120)
    ...
    @field_validator("category")
    @classmethod
    def guard_category(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return v
        if v not in RECEIPT_CATEGORIES:
            raise ValueError(f"category 必須是 {RECEIPT_CATEGORIES} 其一")
...
class MiscReceiptSignRequest(BaseModel):
...
def _to_dict(row: MiscReceipt) -> dict:
    return {
        "id": row.id,
...
def _load_receipt(session, receipt_id: int) -> MiscReceipt:
    row = (
        session.query(MiscReceipt)
        .options(joinedload(MiscReceipt.signer), joinedload(MiscReceipt.created_by))
        .filter(MiscReceipt.id == receipt_id)
        .first()
    )
    if row is None:
        raise HTTPException(status_code=404, detail="收款紀錄不存在")
    return row


# ─── Endpoints ───────────────────────────────────────────────────────────
@router.get("/misc-receipts", response_model=MiscReceiptListOut)
def list_misc_receipts(
    start_date: Optional[date] = Query(None),
    end_date: Optional[date] = Query(None),
    payer_name: Optional[str] = Query(None, max_length=120),
    ...
    current_user: dict = Depends(
        require_staff_permission(Permission.MISC_RECEIPT_READ)
    ),
):
    session = get_session()
    try:
        q = session.query(MiscReceipt).options(
            joinedload(MiscReceipt.signer), joinedload(MiscReceipt.created_by)
        )
        filters = []
        if start_date:
            filters.append(MiscReceipt.receipt_date >= start_date)
        if end_date:
            filters.append(MiscReceipt.receipt_date <= end_date)
        if payer_name:
            safe = escape_like_pattern(payer_name.strip())
            filters.append(
                MiscReceipt.payer_name.ilike(f"%{safe}%", escape=LIKE_ESCAPE_CHAR)
            )
        if category:
            filters.append(MiscReceipt.category == category)
        if status:
            filters.append(MiscReceipt.status == status)
        if payment_method:
            filters.append(MiscReceipt.payment_method == payment_method)
        if filters:
            q = q.filter(and_(*filters))

        total = q.count()
        rows = (
            q.order_by(MiscReceipt.receipt_date.desc(), MiscReceipt.id.desc())
...
@router.get("/misc-receipts/summary", response_model=MiscReceiptSummaryOut)
def misc_receipts_summary(
    start_date: Optional[date] = Query(None),
    end_date: Optional[date] = Query(None),
    payer_name: Optional[str] = Query(None, max_length=120),
    category: Optional[str] = Query(None),
    ...
    try:
        ...
        q = session.query(
            MiscReceipt.status,
            func.count(MiscReceipt.id),
            func.coalesce(func.sum(MiscReceipt.amount), 0),
        )
        if filters:
            q = q.filter(and_(*filters))
        rows = q.group_by(MiscReceipt.status).all()
...
@router.get("/misc-receipts/{receipt_id}", response_model=MiscReceiptOut)
def get_misc_receipt(
    receipt_id: int,
    current_user: dict = Depends(
        require_staff_permission(Permission.MISC_RECEIPT_READ)
    ),
    ...
    session = get_session()
    try:
        return _to_dict(_load_receipt(session, receipt_id))
    finally:
        session.close()


@router.post("/misc-receipts", status_code=201, response_model=MutationResultOut)
def create_misc_receipt(
    payload: MiscReceiptCreate,
    current_user: dict = Depends(
        require_staff_permission(Permission.MISC_RECEIPT_WRITE)
    ),
):
    session = get_session()
    try:
        ...
        session.rollback()
        logger.exception("建立雜項收款失敗")
        raise_safe_500(e, context="建立失敗")
    finally:
        session.close()


@router.put("/misc-receipts/{receipt_id}", response_model=DeleteResultOut)
def update_misc_receipt(
    receipt_id: int,
    payload: MiscReceiptUpdate,
    current_user: dict = Depends(
        require_staff_permission(Permission.MISC_RECEIPT_WRITE)
    ),
):
    ...
    try:
        ...
        session.rollback()
        logger.exception("更新雜項收款失敗")
        raise_safe_500(e, context="更新失敗")
    finally:
        session.close()


@router.delete("/misc-receipts/{receipt_id}", response_model=DeleteResultOut)
def delete_misc_receipt(
    receipt_id: int,
    current_user: dict = Depends(
        require_staff_permission(Permission.MISC_RECEIPT_WRITE)
    ),
    ...
    try:
        ...
        session.rollback()
        logger.exception("刪除雜項收款失敗")
        raise_safe_500(e, context="刪除失敗")
    finally:
        session.close()


@router.post("/misc-receipts/{receipt_id}/sign", response_model=DeleteResultOut)
def sign_misc_receipt(
    receipt_id: int,
    payload: MiscReceiptSignRequest,
    request: Request,
    current_user: dict = Depends(
        require_staff_permission(Permission.MISC_RECEIPT_WRITE)
    ),
    ...
    try:
        ...
        request.state.audit_entity_id = str(receipt_id)
        request.state.audit_summary = f"簽收雜項收款 #{receipt_id}"
        return {"message": "簽收成功"}
    except HTTPException:
        raise
    except Exception as e:
...
@router.get("/misc-receipts/{receipt_id}/signature")
...
@router.post(
    ...
    status_code=201,
    response_model=MiscReceiptAttachmentMetaOut,
)
async def upload_attachment(
    ...
    try:
        row = _load_receipt(session, receipt_id)
        # P3-3(b)：已簽收（終態）收款不可增刪附件，對齊
        # update/delete/sign 的 status 守衛，防簽收後抽換/刪除憑據佐證（不可否認性）。
        if row.status != "pending":
            raise HTTPException(status_code=409, detail="已簽收的收款不可變更附件")
        existing = list(row.attachments or [])
        if len(existing) >= MAX_ATTACHMENTS_PER_RECEIPT:
            raise HTTPException(
                status_code=400,
                detail=f"附件數量已達上限（{MAX_ATTACHMENTS_PER_RECEIPT}）",
            )
        ...
        request.state.audit_entity_id = str(receipt_id)
        request.state.audit_summary = f"上傳雜項收款附件 #{receipt_id}：{filename}"
        return meta
    except HTTPException:
        raise
    except Exception as e:
...
@router.delete(
    "/misc-receipts/{receipt_id}/attachments", response_model=DeleteResultOut
)
def delete_attachment_endpoint(
    ...
    try:
        row = _load_receipt(session, receipt_id)
        # P3-3(b)：已簽收（終態）收款不可增刪附件（同 upload 守衛）。
        if row.status != "pending":
            raise HTTPException(status_code=409, detail="已簽收的收款不可變更附件")
        ...
        request.state.audit_entity_id = str(receipt_id)
        request.state.audit_summary = f"刪除雜項收款附件 #{receipt_id}"
...
@router.get("/misc-receipts/{receipt_id}/attachments/download")
