...
def get_festival_bonus_base(position: str, role: str, bonus_base: dict) -> float:
    ...
    grade = get_position_grade(position, POSITION_GRADE_MAP)
...
def get_office_festival_bonus_base(position: str, title: str, office_map: dict) -> Optional[float]:
    """
    取得司機/美編節慶獎金基數
...
def is_eligible_for_festival_bonus(hire_date, reference_date=None, festival_months: int = 3) -> bool:
    """
...
def calculate_overtime_bonus(
    ...
    # 取得超額目標人數
    overtime_target = get_overtime_target(grade_name, has_assistant, is_shared_assistant, overtime_target_map)

    # 計算超額人數
    overtime_count = max(0, current_enrollment - overtime_target)

    # 取得每人金額
    per_person = get_overtime_per_person(role_for_bonus, grade_name, overtime_per_person_map)
...
def calculate_festival_bonus_v2(
    ...
    # 取得節慶獎金目標人數
    target = get_target_enrollment(grade_name, has_assistant, is_shared_assistant, target_enrollment_map)
