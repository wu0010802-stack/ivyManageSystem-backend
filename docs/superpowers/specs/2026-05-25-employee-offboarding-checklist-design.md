...
def upgrade():
    ...
    op.create_index("ix_offboarding_open_status",
                    ...
                    postgresql_where=text("closed_at IS NULL"))
    # 一併加 SalaryRecord.unused_leave_payout 新欄位
    op.add_column("salary_records",
                  sa.Column("unused_leave_payout", sa.Numeric(12, 2),
                            nullable=False, server_default="0",
                            comment="特休未休折現（§38；獨立 column 不進 gross_salary，仿 appraisal_year_end_bonus 模式）"))
...
**`unused_leave_payout` 為新欄位**：仿 `appraisal_year_end_bonus`（`models/salary.py:242`）模式 — 獨立 column 不進 `gross_salary`，避免薪資引擎重複加總。前端 SalaryView 顯示但不重算，二代健保補充保費自動偵測（已存在邏輯）。

**特休餘額計算：** 無 `LeaveBalance` 表；reuse `api/leaves_quota.py:234` `_get_used_hours()` 邏輯（公式 `quota.total_hours - used_hours_approved`），抽到 `utils/leave_quota_helpers.py` public function `get_annual_leave_balance(session, employee_id, snapshot_date) -> dict`。
...
|---|---|---|---|
| `POST` | `/offboarding/{employee_id}/preview` | `EMPLOYEES_WRITE` | 1 |
| `POST` | `/offboarding/{employee_id}/process` | `EMPLOYEES_WRITE` | 1 |
| `GET`  | `/offboarding/{employee_id}` | `EMPLOYEES_READ` | 1 |
| `GET`  | `/offboarding/{employee_id}/certificate.pdf` | `EMPLOYEES_READ` 或 admin self | 2 |
| `PATCH` | `/offboarding/{employee_id}/nhi-unenroll` | `EMPLOYEES_WRITE` | 1 |
| `POST` | `/offboarding/{employee_id}/magic-link` | `EMPLOYEES_WRITE` | 2 |
| `DELETE` | `/offboarding/{employee_id}/magic-link` | `EMPLOYEES_WRITE` | 2 |
| `GET`  | `/offboarding/download` | 公開（IP rate limit） | 2 |
| `GET`  | `/offboarding/` | `EMPLOYEES_READ` | 3 |
...
**清單頁：** AdminSidebar「人事管理 → 離職管理」新菜單項，gated by `EMPLOYEES_READ`。
...
使用既有 `EMPLOYEES_READ`、`EMPLOYEES_WRITE` IntFlag bit，**不**新增 permission bit（避免 32-bit 邊界又加負擔）。
...
| Phase 2 magic-link 前端未 ship 前 admin 無法用 | Phase 2 ship 後 UX 有空窗 | Phase 3 同 PR ship 前端 magic-link panel；Phase 2 上線後 admin 可 curl 暫用（已有 `EMPLOYEES_WRITE` admin 教學）|
