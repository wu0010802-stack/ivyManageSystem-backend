...
def _resolve_comp_quota_row(
    session, employee_id: int, target_date, *, lock: bool = False
):
    """解析補休 LeaveQuota 列：優先學年制列、fallback legacy 西元年列。

    與 api.leaves_quota._resolve_quota_row（配額檢查的讀路徑）對齊，確保補休發放/撤銷
    寫入的列 == 配額檢查讀取的列（rank 15）。學年 cutover 後，發放若仍只寫 legacy 西元
    年列、檢查卻優先讀 school_year 列 → 命中不同列 → 合法補休被誤擋。

    回傳 (row_or_None, school_year)。lock=True 取 with_for_update 列鎖（grant 發放用，
    對齊原 _grant_comp_leave_quota 的併發保護）。
    """
    ...
    q1 = session.query(LeaveQuota).filter(
    ...
    q2 = session.query(LeaveQuota).filter(
        LeaveQuota.employee_id == employee_id,
        LeaveQuota.leave_type == "compensatory",
...
def _revoke_comp_leave_grant(
    ...
    # rank 15：用與配額檢查相同的列解析（學年優先、legacy fallback），避免 cutover 後
    # 撤銷扣到的列與檢查讀的列不一致。
    quota, _sy = _resolve_comp_quota_row(session, ot.employee_id, ot.overtime_date)
...
def _grant_comp_leave_quota(session, ot: OvertimeRecord, result: dict) -> None:
    ...
    if not (ot.use_comp_leave and not ot.comp_leave_granted):
        return
    year = ot.overtime_date.year
    # rank 15：用與配額檢查相同的列解析（學年優先、legacy fallback）+ 列鎖，確保發放
    # 寫入的列 == 檢查讀取的列；無列時新建「學年制」列（與檢查讀路徑優先學年一致）。
    quota, school_year = _resolve_comp_quota_row(
        session, ot.employee_id, ot.overtime_date, lock=True
    )
    if quota:
