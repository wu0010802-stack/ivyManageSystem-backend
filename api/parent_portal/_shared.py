...
def _assert_student_owned(
    ...
    *,
    ...
    for_write=True（audit 2026-05-07 P0）：額外擋已退學/畢業/轉出子女的寫入
    動作（投藥單、簽收、新聯絡簿條目、新請假），避免家長對已離校子女持續
    佔用系統資源（用藥彙總/老師待辦）。讀歷史路徑保留 for_write=False。
    """
    ...
    if for_write:
        ...
        if student is not None and student.lifecycle_status in _TERMINAL_LIFECYCLE:
            raise HTTPException(
                status_code=403,
                detail="此學生已離校，無法執行新增/變更動作；可繼續查看歷史紀錄",
            )
