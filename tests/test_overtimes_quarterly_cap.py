...
- rejected (status="rejected") 不算進累計
- 跨年窗口（target=2026-01 → W1=2025/11~2026/01）正確跨年
- pending (status="pending") 算進累計（與 monthly 同口徑）
"""
...
def _add_ot(session, emp_id, ot_date, hours, status="pending"):
...
class TestCheckQuarterlyOvertimeCap:
    ...
    def test_all_windows_pass(self, session):
        ...
        _add_ot(session, 1, date(2026, 3, 10), 15.0, status="approved")
        _add_ot(session, 1, date(2026, 4, 10), 10.0, status="approved")
        _add_ot(session, 1, date(2026, 5, 10), 15.0, status="approved")
    ...
    def test_middle_window_blocks_with_w2_label(self, session):
        ...
        _add_ot(session, 1, date(2026, 3, 5), 5.0, status="approved")
        _add_ot(session, 1, date(2026, 4, 5), 45.0, status="approved")
        _add_ot(session, 1, date(2026, 5, 5), 45.0, status="approved")
        _add_ot(session, 1, date(2026, 6, 5), 45.0, status="approved")
    ...
    def test_exclude_id_excludes_self(self, session):
        ...
        _add_ot(session, 1, date(2026, 3, 5), 50.0, status="approved")
        _add_ot(session, 1, date(2026, 4, 5), 50.0, status="approved")
        old = _add_ot(session, 1, date(2026, 5, 5), 30.0, status="approved")
    ...
    def test_rejected_not_counted(self, session):
        """status="rejected" 不算進累計"""
        _add_ot(session, 1, date(2026, 3, 5), 50.0, status="rejected")
        _add_ot(session, 1, date(2026, 4, 5), 50.0, status="rejected")
        _add_ot(session, 1, date(2026, 5, 5), 50.0, status="rejected")
    ...
    def test_year_boundary_wraps_correctly(self, session):
        ...
        _add_ot(session, 1, date(2025, 11, 5), 45.0, status="approved")
        _add_ot(session, 1, date(2025, 12, 5), 45.0, status="approved")
        _add_ot(session, 1, date(2026, 1, 5), 45.0, status="approved")
        with pytest.raises(HTTPException) as exc:
    ...
    def test_pending_counted(self, session):
        """status="pending" (pending) 算進累計"""
        _add_ot(session, 1, date(2026, 3, 5), 45.0, status="pending")
        _add_ot(session, 1, date(2026, 4, 5), 45.0, status="pending")
        _add_ot(session, 1, date(2026, 5, 5), 45.0, status="pending")
        with pytest.raises(HTTPException):
            check_quarterly_overtime_cap(session, 1, date(2026, 5, 20), 5.0)
    ...
    def test_first_window_wins_when_multiple_exceed(self, session):
        ...
        _add_ot(session, 42, date(2026, 3, 1), 50.0, status="approved")
        _add_ot(session, 42, date(2026, 4, 1), 50.0, status="approved")
        _add_ot(session, 42, date(2026, 5, 1), 50.0, status="approved")
        _add_ot(session, 42, date(2026, 6, 1), 50.0, status="approved")
        with pytest.raises(HTTPException) as exc:
