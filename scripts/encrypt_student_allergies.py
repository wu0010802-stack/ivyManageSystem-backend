"""scripts/encrypt_student_allergies.py — 既有 plaintext StudentAllergy 加密 backfill.

RA-MED-10：把既有 plaintext 的 student_allergies.allergen / reaction_symptom /
first_aid_note 一次性加密為 Fernet ciphertext。severity **不加密**（維持明文，
DB-level ORDER BY 排序需求）。

執行：
    # Dry-run（不寫 DB，只列 row 數）
    python scripts/encrypt_student_allergies.py --dry-run

    # 真實 execute（每批 500 列 + commit）
    python scripts/encrypt_student_allergies.py --execute

設計重點（對齊 scripts/encrypt_medical_fields.py）：
- **Idempotent**：is_encrypted() 已加密則 skip，可重跑
- **Batched**：每批 500 commit，避免長交易鎖表
- **不走 ORM 加密路徑**：raw SQL UPDATE，避免 EncryptedText 透明加密又一輪
- **Migration window 安全**：執行前後 endpoint 都能讀（legacy plaintext fallback）

前置：先跑 `alembic upgrade head`（allergyenc01 把 varchar 欄位放寬為 Text，
否則密文塞不下 allergen varchar(100) / reaction_symptom varchar(200)）。
...
# 要 backfill 的欄位（皆位於 student_allergies 表；severity 刻意不加密）
_TARGET_FIELDS = ("allergen", "reaction_symptom", "first_aid_note")
...
def _count_plaintext_rows(session) -> dict[str, int]:
    ...
    for field in _TARGET_FIELDS:
        rows = session.execute(
            text(
                f"SELECT id, {field} FROM student_allergies "
    ...
    while True:
        ...
        for row_id, value in rows:
            ...
            try:
                ...
                session.execute(
                    text(f"UPDATE student_allergies SET {field} = :ct WHERE id = :id"),
...
def main():
    parser = argparse.ArgumentParser(
        description="Backfill 既有 plaintext StudentAllergy 欄位為 Fernet ciphertext (idempotent)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    logger.info("=" * 50)
    logger.info("encrypt_student_allergies backfill %s", mode)
    logger.info("=" * 50)
