...
- **加班雙重上限**（勞基法 §32 II）：`services/overtime_conflict_service` 同步檢查月度 46h（`check_monthly_overtime_cap`）+ 季度 138h（`check_quarterly_overtime_cap`，曆月對齊 rolling 3 月）。兩者並排呼叫，admin/portal create/update/approve/batch/import 6 個 call site 同步 enforce；越界拒 422。設計規格見 `docs/superpowers/specs/2026-05-26-overtime-quarterly-cap-138h-design.md`
