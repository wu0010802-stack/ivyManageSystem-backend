...
⚠️ 分母接縫（P3-J，2026-06-25 稽核；刻意保留，待業主裁定）：
    本檔底薪折算採「在職曆日 ÷ 當月曆日數（calendar.monthrange，28~31）」——台灣
    月中入退職的標準曆日折算；而每日扣款（曠職/遲到/早退/請假，services/salary/
    deduction.py 與 utils 的 calc_daily_salary）採法定日薪基準「月薪 ÷ 30」
    （constants.MONTHLY_BASE_DAYS=30，勞基法）。兩者皆為合法慣例但分母不同。

    後果：當「月中入退職」與「同月缺勤/請假」同時發生時，折算每日授予值與每日扣款
    值在大小月會有微小落差（31 天月：折算 base/31 < 扣款 base/30 → 缺勤日淨溢扣
    約數十元；28 天月：方向相反、對員工有利）。落差有界（單缺勤日 |base/30 − base/
    月曆日|），且僅在該交集觸發。

    刻意不在此統一分母：① 折算改 /30 會讓非 30 天月的「部分月」偏離曆日折算標準；
    ② 扣款改曆日會推翻法定 /30 日薪基準、影響全體；任一改動都把其中一個合法標準
    改錯且廣泛影響所有月中入退職員工。屬慣例裁量，待業主決定統一基準後再調整。
"""
