...
class MeetingRecord(Base):
    ...
    meeting_type = Column(
        String(30), default="staff_meeting", comment="會議類型: staff_meeting"
    )
...
class SchoolEvent(Base):
    ...
    event_type = Column(
        String(30),
        default="general",
        comment="事件類型: meeting/activity/holiday/general",
    )
...
class AnnouncementRead(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "announcement_id", "employee_id", name="uq_announcement_employee"
        ),
    )
...
class EventAcknowledgment(Base):
    ...
    __table_args__ = (
        UniqueConstraint("event_id", "user_id", "student_id", name="uq_event_ack"),
    ...
    signature_attachment_id = Column(
        Integer,
