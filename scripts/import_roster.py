    ...
    try:
        ...
        def get_or_create_employee(name, title=None, emp_id_prefix="T"):
            ...
            if not emp:
                ...
                emp = Employee(
                    employee_id=emp_id, name=name, title=title, is_active=True
                )
                db.add(emp)
                db.flush()
            return emp
        ...
        for class_name, students in roster.items():
            ...
            for s_name, s_tag in students:
                ...
                existing_s = (
                    db.query(Student)
                    .filter_by(name=s_name, classroom_id=classroom.id)
                    .first()
                )
                if not existing_s:
                    # 找該班目前最大學號
                    count = (
                        db.query(Student).filter_by(classroom_id=classroom.id).count()
                        + 1
                    )
                    ...
                    student = Student(
                        ...
                        is_active=True,
                        enrollment_date=now_taipei_naive().date(),
                    )
                    db.add(student)
                    student_count += 1
