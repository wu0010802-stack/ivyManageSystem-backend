...
4. **切 DATABASE_URL → 重新部署 Zeabur backend**
   ...
   - **DR 重建必檢**：確認 `ADMIN_INIT_USERNAME` + `ADMIN_INIT_PASSWORD` 已設。
     restore 出來的 DB 若無 `role='admin'` 帳號（或本來就空 DB），
     startup 會 fail-fast `RuntimeError` 拒絕啟動（見 `startup/seed.py:seed_default_admin`）。
     healthcheck 持續紅燈 = env 漏設訊號；補上 env 後 Restart 即可。
