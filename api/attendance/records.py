...
async def create_or_update_attendance_record(record: AttendanceRecordUpdate, current_user: dict = Depends(require_staff_permission(Permission.ATTENDANCE_WRITE))):
    """新增或更新單筆考勤記錄"""
    session = get_session()
    try:
...
async def delete_single_attendance_record(employee_id: int, date: str, current_user: dict = Depends(require_staff_permission(Permission.ATTENDANCE_WRITE))):
    """刪除單筆考勤記錄"""
    session = get_session()
    try:
...
def delete_single_attendance(employee_id: int, date_str: str, current_user: dict = Depends(require_staff_permission(Permission.ATTENDANCE_WRITE))):
    """刪除單筆考勤記錄"""
    session = get_session()
    try:
        try:
...
async def delete_attendance_records(year: int, month: int, current_user: dict = Depends(require_staff_permission(Permission.ATTENDANCE_WRITE))):
    """刪除指定月份的所有考勤記錄"""
    session = get_session()
    try:
        start_date = date(year, month, 1)
