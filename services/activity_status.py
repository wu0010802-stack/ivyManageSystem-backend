...
- match_status：unmatched→{matched,pending}（公開報名比對）；
  pending→{matched(rematch),manual(人工綁),rejected(駁回),forced(強行收件)}；
  rejected→{pending}（restore）；matched/manual/forced 視為已解析終態
  （階段 1 soft-enforce 若揭露合法漏邊再補）。
"""
...
class MatchStatus(str, enum.Enum):
    ...
    REJECTED = "rejected"
    MANUAL = "manual"
    # 強行收件（force-accept）：校外生/永遠比對不上但校方決定收件，跳過三欄
    # 比對直接插入正式列表。視為已解析終態（同 matched/manual）。
    FORCED = "forced"
