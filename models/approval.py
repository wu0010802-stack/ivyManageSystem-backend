"""models/approval.py — 多級簽核政策、稽核記錄與審核狀態 enum。

ApprovalPolicy / ApprovalLog：多級簽核政策與稽核記錄。
ApprovalStatus：審核狀態 enum，由 LeaveRecord / OvertimeRecord /
PunchCorrectionRequest 的 `status` String(20) column 採用。

(P1/P2 期間曾透過 SQLAlchemy attribute event listener 在
`is_approved` 與 `status` 之間雙寫；P4 拔除 `is_approved` column
後 listener 不再需要，已隨之移除。)
"""
...
class ApprovalLog(Base):
    ...
    action = Column(
        String(20), nullable=False, comment="操作：approved / rejected / cancelled"
    )
