...
### Phase 1 結果摘要（共 46 筆 finding，F-001~F-046）

| 級別 | 筆數 | 狀態 |
|---|---:|---|
| Critical | 3 | ⏳ Pending（Phase 2） |
| High | 15 | ⏳ Pending |
| Medium | 14 | ⏳ Pending |
| Low | 14 | ⏳ Pending |
...
- **F-037 Critical** `POST /api/auth/users` — USER_MANAGEMENT_WRITE 持有者可建 admin 帳號
- **F-038 Critical** `PUT /api/auth/users/{user_id}` — 自我升 admin
- **F-039 Critical** `PUT /api/auth/users/{user_id}/reset-password` — 重設 admin 密碼後接管

> 三筆共同根因：USER_MANAGEMENT_WRITE 缺 `target.role <= caller.role` 與 `payload.role/permissions <= caller.role/permissions` 的上限守衛。

### Phase 2 規劃

依等級分批修補（Critical → High → Medium → Low），每筆 Critical/High 配 pytest 回歸測試。Phase 2 plan 將另起，路徑：`docs/superpowers/plans/2026-04-XX-idor-fix-phase2.md`（尚未撰寫）。

修補時應抽下列共用 helper：`utils/idor_guards.*`（員工自查/班級 scope/家長綁定）、`utils/attendance_guards.require_not_self_attendance`、`utils/finance_response.mask_salary_fields`。
