...
#   matched=自動匹配成功 / manual=人工綁定 / forced=強行收件（校外生或永遠比對
#   不上但校方決定收件，亦為真實付費在班生）。三者皆視為合法在班。
#   其餘（pending/rejected/unmatched）視為未配對。J 與 unmatched 以此乾淨互斥。
#   業主 2026-06-23 裁定 forced 計入：點名/容量/營收都把它當有效報名（口徑
#   != 'rejected'），獎金口徑須一致，否則老師被少算學生數 → 少領獎金。
_SUCCESS_STATUSES = ("matched", "manual", "forced")
...
class AcaReport:
    ...
    unmatched_count  : 未配對（classroom_id IS NULL，或 match_status 不在 {'matched','manual','forced'}）的報名人次
...
def _count_enrollments(db: Session, *, classroom_id: int, academic_year: int) -> int:
    ...
    僅計合法在班（match_status IN ('matched','manual','forced')）的報名，與 unmatched
...
def _count_unmatched(db: Session, *, academic_year: int) -> int:
    ...
    凡不滿足「classroom_id IS NOT NULL AND match_status IN ('matched','manual','forced')」
