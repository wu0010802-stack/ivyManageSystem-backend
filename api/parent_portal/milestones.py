...
from models.database import Guardian, StudentMilestone
...
async def parent_react(
    ...
    try:
        ...
        # react 時避免 parent_acknowledged_by attribution 被後贏者覆蓋。RLS
        # USING 過濾後再 lock，鎖到的就是 parent 可見的 row。
        m = (
            session.query(StudentMilestone)
        ...
        if m.parent_acknowledged_at is None:
            m.parent_acknowledged_at = now_taipei_naive()
            g = (
