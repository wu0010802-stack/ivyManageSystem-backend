...
`is_row_unrestricted(code=Permission.DISMISSAL_CALLS_*.value)` gate：
...
runtime 行為由 utils/portfolio_access.is_row_unrestricted 與 permscope04 migration backfill
共同保證。
"""
...
def test_dismissal_calls_router_uses_is_row_unrestricted_with_code():
    """api/portal/dismissal_calls.py 4 endpoint 都應有 is_row_unrestricted(code=) gate。"""
    import api.portal.dismissal_calls as mod

    source = inspect.getsource(mod)

    # 必匯入 is_row_unrestricted
    assert (
        "is_row_unrestricted" in source
    ), "api/portal/dismissal_calls.py 應 import is_row_unrestricted"

    # READ endpoint（list / pending-count）必呼叫 is_row_unrestricted(code=Permission.DISMISSAL_CALLS_READ.value)
    assert (
    ...
    # WRITE endpoint（acknowledge / complete）必呼叫 is_row_unrestricted(code=Permission.DISMISSAL_CALLS_WRITE.value)
    assert (
...
def test_dismissal_calls_router_unrestricted_skips_classroom_filter():
    """is_row_unrestricted=True 時必須跳過 _get_teacher_classroom_ids 班級限制。
