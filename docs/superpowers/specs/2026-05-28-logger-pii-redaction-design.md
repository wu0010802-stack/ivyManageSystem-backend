...
- (G2) Filter 內對 `record.msg` formatted result（透過 regex 抓 `\w+=\S+` 形式）執行 key 判 PII → value 替換 `[FILTERED]`
- (G3) Filter 內對 `record.args` 若是 dict 走 `_scrub_mapping`；若是 tuple 對應 format string 找出 PII position 替換
- (G4) Filter 對 `record.exc_info` traceback 內 SQL value / exception args 做 redact
- (G5) `main.py:_configure_logging` attach 到 root handler（同 `RequestIdLogFilter` pattern，跟 RequestIdLogFilter 並列）
- (G6) 既有 36+ logger calls 不改（filter 自動 redact）
- (G7) 零回歸：既有 5582 pytest baseline + 5-6 new test 全綠
- (G8) 同步 `_PII_KEY_SUBSTRINGS` 名單與 Sentry 對齊 — import sentry helper 不另抄
...
class PIIRedactionFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        # 1. record.msg formatted result 做 string scrubbing
        # 重要：在 record 上 cache redacted msg，避免 multi-handler 重複 process
        if isinstance(record.msg, str):
            try:
                formatted = record.getMessage()
                redacted = _redact_string(formatted)
                if redacted != formatted:
                    # msg 改寫為 redacted formatted result，args 清空避免 re-format
                    record.msg = redacted
                    record.args = ()
            except Exception:
                # getMessage() 失敗（args/format mismatch）跳過，原樣放行
                pass

        # 2. record.args 若是 dict 做 _scrub_mapping
        # （步驟 1 已 clear args 為 () 時這裡不會影響）
        if record.args:
