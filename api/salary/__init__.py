...
# POST /salaries/calculate / calculate-async,
# GET /salaries/calculate-jobs/{job_id} 已抽到 .calculate；
# 於檔尾 include_router 接回。
...
# GET /salaries/festival-bonus 與 /salaries/festival-bonus/period-accrual
# 已抽到 .festival，於檔尾 include_router 接回。


# GET /salaries/records 已抽到 .records，於檔尾 include_router 接回。


# PUT /salaries/{record_id}/manual-adjust 已抽到 .manual_adjust，
# 於檔尾以 include_router 接回；同時 re-export 以下 symbol 供既有 test
# 直接 import 維持原 surface：
#   - SalaryManualAdjustRequest
#   - _MANUAL_ADJUST_FIELD_MAX
#   - _parse_if_match


# 單筆薪資查詢/匯出（audit-log/breakdown/field-breakdown/export）
# 已抽到 .detail，於檔尾以 include_router 接回。


# GET /salaries/export-all、history、history-all 已抽到 .records，
# 於檔尾 include_router 接回。
...
# ============ Sub-routers ============
#
# 拆分子 router 收尾。子 router include 順序與本檔內定義端點獨立，
# FastAPI 會合併到同一個 prefix 下。
# - simulate:  POST /salaries/simulate
# - snapshots: GET/POST /salaries/snapshots, /{id}, /{id}/diff
from .calculate import (  # noqa: E402,F401
...
from .manual_adjust import (  # noqa: E402,F401
    ...
    SalaryManualAdjustRequest,
    _parse_if_match,
    router as _manual_adjust_router,
)
