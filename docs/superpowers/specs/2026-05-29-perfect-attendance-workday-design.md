...
- `milestone_detector` 維持純函式：新增參數 `official_workdays: Iterable[date]`，
  由 caller（`auto_milestone.py`）以 workday_rules 算 `[第一筆記錄所在月月初, ref_date]`
  區間後傳入（**整月起算**，使首月工作日集合為整月 → 月中才有記錄的半個月正確判為非
  滿月全勤、不發章）。DB 依賴留在 caller 的 `_official_workdays_in_range` helper。
