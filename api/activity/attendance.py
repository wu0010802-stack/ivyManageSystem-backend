...
def list_sessions(
    ...
    try:
        ...
        total = query.count()
        rows = query.order_by(ActivitySession.session_date.desc(), ActivitySession.id.desc()).offset(skip).limit(limit).all()
        ...
        if session_ids:
            agg_rows = (
                session.query(
                    ...
                    func.sum(case((ActivityAttendance.is_present.is_(True), 1), else_=0)).label("present"),
                )
...
def create_session(
    ...
    session = get_session()
    try:
        course = session.query(ActivityCourse).filter(ActivityCourse.id == body.course_id).first()
        if not course:
...
def get_session_detail(
    ...
    try:
        ...
        if not sess:
            raise HTTPException(status_code=404, detail="找不到場次")
        return _build_session_detail_response(session, sess)
    finally:
        session.close()
