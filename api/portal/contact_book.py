...
def list_classroom_day(
    ...
    try:
        ...
        if not is_row_unrestricted(current_user, code=Permission.PORTFOLIO_READ.value):
...
def batch_upsert(
    ...
    try:
        emp = _get_employee(session, current_user)
        if not is_row_unrestricted(current_user, code=Permission.PORTFOLIO_WRITE.value):
