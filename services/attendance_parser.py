...
class AttendanceParser:
    ...
    def _stitch_overnight_punches(self, employee_df: pd.DataFrame) -> tuple:
        """
    ...
    def _analyze_employee_attendance(
        self, employee_name: str, employee_df: pd.DataFrame
    ) -> AttendanceResult:
        """
        分析單一員工的考勤記錄
        """
        ...
        for check_date in expected_dates:
            ...
            punch_datetimes = (
                sorted(day_records["punch_datetime"].tolist())
            ...
            # 判斷是否跨夜（下班打卡在隔日）
            is_overnight = punch_out_dt is not None and punch_out_dt.date() > check_date
            ...
            if len(punch_datetimes) < 2:
                ...
                if is_overnight:
                    work_end_cmp = datetime.combine(
                        check_date + timedelta(days=1), work_end
                    )
                else:
                ...
                if punch_out_dt < work_end_cmp:
                    ...
                    early_leave_count += 1
                    early_minutes = int(
                        (work_end_cmp - punch_out_dt).total_seconds() / 60
                    )
    ...
    def generate_anomaly_report(
        self, results: Dict[str, AttendanceResult]
    ) -> pd.DataFrame:
        """
        產生異常清單報表
                    ...
                    anomaly_records.append(
                        {
                            ...
                            "日期": detail["date"],
                            "上班打卡": (
                                detail["punch_in"].strftime("%H:%M")
                                if detail["punch_in"]
                                else "未打卡"
                            ),
                            "下班打卡": (
                                detail["punch_out"].strftime("%H:%M")
                                if detail["punch_out"]
                                else "未打卡"
                            ),
                            "狀態": detail["status"],
...
def parse_attendance_file(
    ...
    results = parser.parse_attendance_excel(
        file_path, name_column=name_column, datetime_column=datetime_column
    )
