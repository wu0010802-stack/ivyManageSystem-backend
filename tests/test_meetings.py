...
class TestEnforceAbsentNoOvertime:
    ...
    def test_absent_record_overtime_cleared(self):
        ...
        record = MockMeetingRecord(attended=False, overtime_hours=2.0, overtime_pay=200.0)
    ...
    def test_attended_record_overtime_preserved(self):
        ...
        record = MockMeetingRecord(attended=True, overtime_hours=2.0, overtime_pay=200.0)
        _enforce_absent_no_overtime(record)
...
class TestUpdateMeetingBusinessRule:
    ...
    Bug 情境：
      ...
      - 管理員只傳 attended=False，不傳 overtime_pay
      - Bug：overtime_pay 仍殘留 200 → 缺席者白領加班費
      - Fix：attended 改為 False 後，強制歸零
    """

    def _simulate_update(self, record, attended=None, overtime_hours=None, overtime_pay=None):
        """模擬 update_meeting 端點的欄位更新邏輯（含修復後的歸零邏輯）。"""
    ...
    def test_change_attended_to_true_allows_overtime(self):
        """缺席→出席：可以設定加班費"""
        record = MockMeetingRecord(attended=False, overtime_hours=0, overtime_pay=0)
        self._simulate_update(record, attended=True, overtime_pay=200)
        assert record.attended is True
        assert record.overtime_pay == 200

    def test_update_remark_only_keeps_overtime_if_attended(self):
        """只改備註不改 attended：出席者加班費保持不變"""
        record = MockMeetingRecord(attended=True, overtime_hours=2.0, overtime_pay=200.0)
        self._simulate_update(record)  # attended/overtime 都不傳
        assert record.overtime_pay == 200.0

    def test_send_attended_false_with_nonzero_overtime_still_zeros(self):
        """
        即使 client 同時送 attended=False + overtime_pay=100，
        業務規則仍應強制歸零（防止前端 bug 或惡意請求）
        """
        record = MockMeetingRecord(attended=True, overtime_hours=2.0, overtime_pay=200.0)
        self._simulate_update(record, attended=False, overtime_pay=100)
        assert record.overtime_pay == 0
        assert record.overtime_hours == 0
