    ...
    if not policy:
        ...
        if approver_role == "admin":
            logger.warning(
                ...
                doc_type, submitter_role, approver_role,
            )
            return True
        logger.warning(
        ...
        return False
    return approver_role in [r.strip() for r in (policy.approver_roles or "").split(",")]


def _write_approval_log(doc_type: str, doc_id: int, action: str, approver: dict, comment: str | None, session):
    """寫入簽核記錄。日誌寫入失敗時記錄 warning，不阻礙核准主流程。"""
    try:
...
def _get_finalized_salary_record(session, employee_id: int, year: int, month: int):
    ...
    """
    return session.query(SalaryRecord).filter(
