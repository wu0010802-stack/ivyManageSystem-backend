"""HEALTH-MEDICATION scope_options seed + teacher backfill (Phase 2.2)

Revision ID: permscope03
Revises: mergeheads07
Create Date: 2026-06-01

本 migration 將 5 個 HEALTH-MEDICATION permission code 標記為 scope-aware：
...
並把現有 teacher role / teacher users 的 bare HEALTH-MEDICATION codes 升級為 :own_class。
...
downgrade 範圍只限 5 條 HEALTH-MEDICATION codes，不會碰到 STUDENTS_*:own_class（permscope01
管轄）或 PORTFOLIO_*:own_class（permscope02 管轄）。
"""
...
down_revision: Union[str, Sequence[str], None] = "mergeheads07"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    # 1. seed：5 個 HEALTH-MEDICATION codes 標記為 scope-aware
    ...
    # 2. teacher role：bare HEALTH-MEDICATION → :own_class（只改 code='teacher' AND is_core=true）
    if dialect == "postgresql":
    ...
    # 3. teacher users：bare HEALTH-MEDICATION → :own_class + bump token_version
    #    （跳過 wildcard '*' 持有者；其他 code（含 STUDENTS_*:own_class / PORTFOLIO_*:own_class）
    #    原樣保留）
    if dialect == "postgresql":
        op.execute(sa.text(f"""
            UPDATE users
...
def downgrade() -> None:
    ...
    # downgrade 只剝 HEALTH-MEDICATION 後綴，不可動 STUDENTS_*:own_class（permscope01）
    # 與 PORTFOLIO_*:own_class（permscope02）
    scoped_health_codes = tuple(f"{c}:own_class" for c in SCOPE_AWARE_CODES)
    codes_sql = ", ".join(f"'{c}'" for c in SCOPE_AWARE_CODES)
    scoped_sql = ", ".join(f"'{c}'" for c in scoped_health_codes)

    if dialect == "postgresql":
        # 1. teacher users：只剝 HEALTH-MEDICATION:own_class，其他 :own_class 保留
        op.execute(f"""
            UPDATE users
            SET permission_names = ARRAY(
                SELECT CASE
            ...
            WHERE role = 'teacher'
        """)
        # 2. teacher role：只剝 HEALTH-MEDICATION:own_class
        op.execute(f"""
            UPDATE roles
            SET permissions = ARRAY(
                SELECT CASE
            ...
            WHERE code = 'teacher' AND is_core = true
        """)
        # 3. unseed scope_options on 5 條 HEALTH-MEDICATION codes
        op.execute(f"""
            UPDATE permission_definitions
