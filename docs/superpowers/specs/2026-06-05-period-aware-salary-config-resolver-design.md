...
- **R2 — 年終學年（民國）→ BonusConfig 西元 config_year 映射【業主待確認】**：`config_year` 已確認為西元；
  但年終以**民國學年**運作（學年 N = 西元 N+1911 之 8 月 ~ N+1912 之 7 月，跨兩個曆年），
  而月薪路徑用「結算月所屬西元年」很直接。年終 builder（含 `festival_base_for_role`，現為
  `is_active + id.desc()`）呼叫 resolver 時要傳哪個西元年需訂規則。
  **建議預設**：`config_year = 年終結算/發放所屬西元年`（即學年 N → 西元 N+1912，因年終通常於學年結束後次年初發放）。
  **此映射需業主確認**（與本專案其他年終語意一致，皆經業主簽核）；確認前年終站點可先維持現行 `id.desc()`
  並標 TODO，月薪路徑先落地（兩者解耦，互不阻塞）。
